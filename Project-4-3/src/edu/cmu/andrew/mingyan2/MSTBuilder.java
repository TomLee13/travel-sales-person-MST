...
public class MSTBuilder {
	...
	Graph buildGraph(List<String> records, double[][] distMatrix) {
		...
		for (int i = 0; i < recordsArr.length; i++) {
			...
			// System.out.println(vertex.index + ": lon: " + vertex.lon + " lat: " +
			// vertex.lat);
	...
	int[] prim(Graph g) {
		...
		int[] parent = new int[g.getVertices().length];

		// store the vertices in a min heap
		Vertex[] vs = g.getVertices();
		MinHeap keys = new MinHeap(vs);
		...
		while (!keys.isEmpty()) {
			...
			for (int i = 0; i < vs.length; i++) {
				if (u.index != vs[i].index) {
					Vertex v = vs[i];

					int n = keys.countains(v);
					if (n >= 0 && g.edges[u.index][v.index] < keys.get(n).getKey()) {
