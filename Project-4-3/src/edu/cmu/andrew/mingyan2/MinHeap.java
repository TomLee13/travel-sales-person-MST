...
public class MinHeap {
	...
	public MinHeap(Vertex[] vertices) {
		...
		for (int i = 0; i < vertices.length; i++) {
			vs[i].setKey(Double.POSITIVE_INFINITY);
		}
	}
	...
	private void heapify(int i) {
		...
		if (leftChild < size) {
			if (vs[leftChild].getKey() < vs[i].getKey()) {
				smallest = leftChild;
			} else {
				smallest = i;
			}
		...
		if (rightChild < size) {
			if (vs[rightChild].getKey() < vs[smallest].getKey()) {
				smallest = rightChild;
			}
		}
	...
	public void printHeap() {
		for (int i = 0; i < size; i++) {
			System.out.println("Key of " + i + " is " + vs[i].getKey());
		}
	}
}
